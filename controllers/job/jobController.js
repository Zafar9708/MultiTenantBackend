...
const postJob = async (req, res) => {
    try {
        ...
        // ADD NOTIFICATION CODE HERE - RIGHT AFTER JOB IS SAVED
        if (req.user.role === 'recruiter') {
    try {
        const notificationMessage = `Recruiter ${req.user.username} created a new job: ${savedJob.jobTitle} (${savedJob.jobName})`;
...
const updateJob = async (req, res) => {
    try {
        ...
        await job.save();
         ...
         if (req.user.role === 'recruiter') {
            try {
                const notificationMessage = `Recruiter ${req.user.username} updated job: ${job.jobTitle} (${job.jobName})`;
                ...
                console.error('Failed to create notification:', notificationError);
                // Don't throw error - job was updated successfully
            }
        }
